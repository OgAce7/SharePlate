...
def optimize_routes(
    ...
    if not ORTOOLS_AVAILABLE:
        ...
        if not matches.empty:
            truck_route = VehicleRoute(vehicle_id=fleet[0].vehicle_id if fleet else "TRUCK01")
            cum_dist = 0.0
            last_lat, last_lon = depot_lat, depot_lon
            current_time = 0
            for idx, row in matches.iterrows():
                d1 = haversine_km(last_lat, last_lon, row["latitude"], row["longitude"])
                current_time += int((d1 / AVG_SPEED_KMPH) * 60) + SERVICE_TIME_MIN
                truck_route.stops.append(RouteStop(
                    ...
                    arrival_min=current_time, load_after_stop=row["quantity"]
                ))
                d2 = haversine_km(row["latitude"], row["longitude"], row["ngo_latitude"], row["ngo_longitude"])
                current_time += int((d2 / AVG_SPEED_KMPH) * 60) + SERVICE_TIME_MIN
                truck_route.stops.append(RouteStop(
                    node_type="delivery", donation_id=row["donation_id"], ngo_id=row["ngo_id"],
                    name=f"Deliver to {row.get('ngo_name', row['ngo_id'])}", latitude=row["ngo_latitude"], longitude=row["ngo_longitude"],
                    arrival_min=current_time, load_after_stop=0
                ))
                cum_dist += (d1 + d2)
                last_lat, last_lon = row["ngo_latitude"], row["ngo_longitude"]
            truck_route.total_distance_km = round(cum_dist, 2)
            truck_route.total_load = 0.0
